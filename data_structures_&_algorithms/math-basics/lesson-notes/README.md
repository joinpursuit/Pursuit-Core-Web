...
```js
// Subtract the value of total times .1 from the total
```
...
In this case we can use the mnemonic PEMDAS
...
Therefore, we don't need to add parenthesis
...
- subtract our percentage from 1

```js
let ductTapePrice = 5;
let amountOfDuctTape = 4;
let percentage = .1
let getDiscountedTotal = 1 - .9
...
console.log(comparePrices(competitorPrices))
```
...
The default sort method, sorts by the first number only
...
`Math.pow ``**` - raise any number to any power
