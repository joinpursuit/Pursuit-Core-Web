...
A resource may also contain sub-collection resources. For example, sub-collection resource “sub-breeds” of a particular “breed” can be identified using the URL `/breeds/{breedName}/sub-breeds` (in the Dog API domain). Similarly, a singleton resource “subBreedName” inside the sub-collection resource “sub-breeds” can be identified as follows: `/breeds/{breedName}/sub-breed/{subBreedName}`.
