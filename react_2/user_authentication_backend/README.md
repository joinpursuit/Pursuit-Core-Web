...
- `passport-local` is the specific _strategy_ we're applying to our Passport authentication. There are many different strategies you can employ to manage your sessions. For example, the `passport-google` package lets you authenticate to other sites using your Google login. _Local_ means that we're handling everything in-house (as opposed to using another service, like Firebase) - we're managing usernames and passwords, we're giving session tokens, and we're processing registration and authentication.
...
Here's the general process of what needs to happen for a user to be authenticated. It's fairly involved so it's good to have a high level understanding before we dive into the code!
...
- `local.js` is where we define our `strategy` for passport. It determines whether or not we will, in fact, assign a session token to a user who is logging in - in other words, we make sure to check whether the user exists in the database and whether the provided password is correct.
